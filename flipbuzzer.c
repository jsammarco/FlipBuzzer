...
#define FLIPBUZZER_PLAYBACK_TICK_MS 5U
...
static uint32_t flipbuzzer_dual_tone_next_frequency(FlipBuzzerApp* app, uint32_t freq1, uint32_t freq2) {
    ...
    if(freq1 == 0) return 0;
    if(freq2 == 0) return freq1;

    uint32_t frequency = app->dual_tone_phase_high ? freq2 : freq1;
    app->dual_tone_phase_high = !app->dual_tone_phase_high;
    return frequency;
}
...
static void flipbuzzer_output_apply(FlipBuzzerApp* app, uint32_t frequency, uint8_t duty) {
    furi_assert(app);
...
static void flipbuzzer_pwm_start(FlipBuzzerApp* app, uint32_t frequency, uint8_t duty) {
    furi_assert(app);
    app->dual_tone_phase_high = false;
    flipbuzzer_output_apply(app, frequency, duty);
}

static void flipbuzzer_pwm_start_dual(
    FlipBuzzerApp* app,
    uint32_t freq1,
    uint32_t freq2,
    uint8_t duty) {
...
static void flipbuzzer_file_playback_apply_step(FlipBuzzerApp* app) {
    ...
    if(step->freq1 == 0 || step->duty == 0) {
        flipbuzzer_pwm_stop(app);
    } else {
        app->dual_tone_phase_high = false;
        flipbuzzer_pwm_start_dual(app, step->freq1, step->freq2, step->duty);
    }
}
...
static bool flipbuzzer_parse_frequency_pair(
    const char* token,
    uint32_t* freq1,
    uint32_t* freq2) {
    if(!token || !freq1 || !freq2 || !*token) return false;

    const char* plus = strchr(token, '+');
    if(!plus) {
        *freq2 = 0;
        return flipbuzzer_parse_u32_token(token, freq1);
    }

    size_t first_len = (size_t)(plus - token);
    size_t second_len = strlen(plus + 1);
    if(first_len == 0 || second_len == 0 || first_len >= 16 || second_len >= 16) return false;

    char first[16];
    char second[16];
    memcpy(first, token, first_len);
    first[first_len] = '\0';
    memcpy(second, plus + 1, second_len + 1);

    return flipbuzzer_parse_u32_token(first, freq1) && flipbuzzer_parse_u32_token(second, freq2);
}
        ...
        while(line && parsed_count < FLIPBUZZER_MAX_NOTES) {
            ...
            if(*line && *line != '#') {
                ...
                if(token_count >= 2) {
                    ...
                    if(flipbuzzer_parse_frequency_pair(tokens[0], &freq1, &freq2) &&
                       flipbuzzer_parse_u32_token(tokens[1], &duration_ms)) {
                        ...
                        if(duration_ms > 0) {
                            if(freq1 > FLIPBUZZER_MAX_FREQ) freq1 = FLIPBUZZER_MAX_FREQ;
                            if(freq2 > FLIPBUZZER_MAX_FREQ) freq2 = FLIPBUZZER_MAX_FREQ;
                            if(duty < FLIPBUZZER_MIN_DUTY) duty = FLIPBUZZER_MIN_DUTY;
                            if(duty > FLIPBUZZER_MAX_DUTY) duty = FLIPBUZZER_MAX_DUTY;

                            out_steps[parsed_count].freq1 = (uint16_t)freq1;
                            out_steps[parsed_count].freq2 = (uint16_t)freq2;
...
static void flipbuzzer_tick_callback(void* context) {
    ...
    const FlipBuzzerToneStep* step = &app->file_steps[app->file_step_index];
    if(step->freq1 > 0 && step->freq2 > 0 && step->duty > 0) {
        flipbuzzer_pwm_start_dual(app, step->freq1, step->freq2, step->duty);
    }
...
static FlipBuzzerApp* flipbuzzer_alloc(void) {
    ...
    furi_assert(app);
    memset(app, 0, sizeof(FlipBuzzerApp));
